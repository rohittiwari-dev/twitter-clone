	...
	return (
		<div
			className={
				!sidebar
					? styles.wrapper
					: `${styles.wrapper} overflow-y-hidden`
			}
		>
			<div
				...
				<div className={styles.header}>
					...
					<div className={styles.headerTitle}>
						...
						<div className="w-6 h-6 relative sm:hidden ">
							<VscTwitter className="w-8 h-8 text-[#1d9bf0]" />
						</div>
					</div>
					<BsStars />
				</div>
				<div className={styles.headerTab}>
					<div
						onClick={(e) => {
							...
							setFeedtype("foryou");
							document.getElementById("againstyou").className =
								styles.headerButton;
						}}
					...
					<div
						onClick={(e) => {
							...
							document.getElementById("foryou").className =
								styles.headerButton;
						}}
			...
			{!loading ? (
				tweetPost.map((tweet, index) => {
					const postOwner = postOwners.find(
						(owner) => owner.email === tweet.postOwner,
					);
					if (tweet && postOwner)
						return (
							<Posts
