...
const Posts = ({
	...
	const deletePost = async () => {
		if (user.tweets.includes(tweetId)) {
			...
			if (mainTweet) {
				mainTweet.comments.forEach(async (element) => {
					await deleteDoc(doc(dataBase, "comments", element));
								...
								{showModel && (
									<Modal closeModel={closeModel}>
										...
										>
											...
											<div className="bg-slate-400/50 w-[1px] h-full absolute top-8 left-8 z-[0]"></div>
											<div className={styles.postMain}>
												<div>
