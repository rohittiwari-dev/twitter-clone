...
const Feed = ({ user }) => {
	...
	return (
		...
		>
			<div
				className={`absolute top-0 left-0 bg-black shadow-lg  shadow-slate-900 z-50 transition ease-in-out duration-700 ${
					sidebar ? "max-w-fit" : "w-0 overflow-hidden"
				}`}
			>
				<Sidebar user={user} mobile={true} activeLink={"Home"} />
			</div>
